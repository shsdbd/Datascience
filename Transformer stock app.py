...
CONFIG = {
    ...
    'learning_rate': 0.001,
    'batch_size': 64,
    'static_input_dim': 3,    # 고정 Feature 개수 (PER, PBR, ROE)
    'static_hidden_dim': 32,  # 고정 데이터를 처리할 Linear Layer의 크기
...
# ---------------------------------------------------------
# 3. Hybrid Transformer Encoder 모델 정의
# ---------------------------------------------------------
class HybridTransformer(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers, output_dim, dropout, static_input_dim, static_hidden_dim):
        ...
        # 6. 최종 출력층
        self.decoder = nn.Linear(d_model + static_hidden_dim, output_dim)
        
        self.d_model = d_model

    def forward(self, x_seq, x_static):
        # x_seq: (Batch, Seq, Input_Dim)
        
        # Embedding & Scaling
        x_seq = self.embedding(x_seq) * math.sqrt(self.d_model)
        
        # Positional Encoding 더하기
        x_seq = self.pos_encoder(x_seq)
        
        # Transformer Encoder 통과
        output_seq = self.transformer_encoder(x_seq)
        
        # 예측을 위해 마지막 시점(t)의 output만 사용
        transformer_last_output = output_seq[:, -1, :] # (Batch, d_model)
        
        # 고정 데이터 처리
        static_out = self.static_layer(x_static) # (Batch, StaticHidden)

        # 데이터 결합 (Concatenate)
        combined = torch.cat((transformer_last_output, static_out), dim=1)
...
def train_model(model, X_train_seq, X_train_static, y_train, config):
    ...
    model.train()
    print(f"\nStarting Training (Hybrid Transformer) with {len(data_loader.dataset)} samples...")
...
def validate_and_plot_ticker(model, ticker, processor, config, timestamp):
    ...
    plt.title(f"{ticker} Stock Prediction (Hybrid Transformer) - Corrected")
    plt.xlabel('Time (Test Data Index)')
    plt.ylabel('Stock Price')
    plt.legend()
    plt.grid(True)
    
    plot_filename = f"validation_plot_HybridTransformer_{ticker}_{timestamp}.png"
